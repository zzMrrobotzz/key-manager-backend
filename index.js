...
app.get('/api/stats/dashboard', async (req, res) => {
  try {
    ...
    res.json({
      billingStats: { totalRevenue: totalRevenue[0]?.total || 0, monthlyTransactions, pendingRevenue: 0, successRate: 100 },
      apiUsageStats: { totalRequests: apiUsage[0]?.totalRequests || 0, todayRequests: 0, averageCost: 0, errorRate: 0 }
    });
  } catch (error) {
...
app.post('/api/usage/log', async (req, res) => {
    ...
    if (!providerName) return res.status(400).json({ message: 'Thiếu tên nhà cung cấp' });
    try {
        await ApiProvider.updateOne({ name: providerName }, { $inc: { totalRequests: 1, costToday: cost || 0 }, $set: { lastChecked: Date.now() } });
...
app.post('/api/providers/:providerId/keys', async (req, res) => {
    ...
    try {
        ...
        if (provider.apiKeys.includes(apiKey)) return res.status(409).json({ message: 'API key already exists' });
...
app.delete('/api/providers/:providerId/keys', async (req, res) => {
    ...
    try {
        ...
        if (keyIndex === -1) return res.status(404).json({ message: 'API key not found' });
...
app.post('/api/payment/create', async (req, res) => {
  try {
    ...
    if (!key || !credit || credit < 1) return res.status(400).json({ success: false, message: 'Thiếu key hoặc số credit không hợp lệ' });
    ...
    const payosData = { orderCode, amount, description, buyerName: key, returnUrl, cancelUrl, signature };
    const { data: payosRes } = await axios.post(PAYOS_API_URL, payosData, { headers: { 'x-client-id': PAYOS_CLIENT_ID, 'x-api-key': PAYOS_API_KEY } });
...
app.post('/api/payment/webhook', async (req, res) => {
    ...
    try {
        if (webhookData.code === '00' && webhookData.data.status === 'PAID') {
            ...
            if (existingTransaction) return res.status(200).json({ message: 'Transaction already processed.' });
            const Key = require('./models/Key');
            await Key.updateOne({ key }, { $inc: { credit: credit } });
            const newTransaction = new Transaction({ orderId: orderCode, amount, creditAmount: credit, key, status: 'Success' });
